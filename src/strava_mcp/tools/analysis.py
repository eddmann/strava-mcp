...
async def analyze_training(
    period: Annotated[
        TimeRangePreset | str,
        "Time period to analyze. Preset: '7d', '30d', '90d', 'ytd', 'this-week', "
        "'this-month', or custom range 'YYYY-MM-DD:YYYY-MM-DD'",
    ] = "30d",
        ...
        for activity in activities:
            ...
            if "distance" in criteria_list:
                ...
                if reference.distance > 0:
                    distance_diff = abs(activity.distance - reference.distance) / reference.distance
            ...
            if "duration" in criteria_list:
                ...
                if reference.moving_time > 0:
                    time_diff = (
                        abs(activity.moving_time - reference.moving_time) / reference.moving_time
                    )
                    if time_diff <= 0.15:
                        score += 1
                    elif time_diff <= 0.3:
